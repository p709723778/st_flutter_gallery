...
void _sms4Crypt(List<int> input,
    List<int> output,
    List<int> roundKey,) {
...
dynamic _sm4Factory({
  ...
  List<int> outArray = List.filled(isUTF8 ? 32 : 16, 0);
  ...
  if (outputType != SM4OutputMode.Array) {
    if (cryptFlag != DECRYPT) {
      ...
      return isUTF8 ? _listToUtf8(outArray) : _listToHex(outArray);
    }
  } else {
    return outArray;
  }
...
class SM4 {
  ...
  static String encrypt({
    ...
    // hex data
    List<int> dataHexList = isUTF8 ? _utf8ToArray(data) : _hexToArray(data);
