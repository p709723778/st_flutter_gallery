...
class _ScanResultTileState extends State<ScanResultTile> {
  ...
  Widget _buildConnectButton(BuildContext context) {
    return ElevatedButton(
      ...
      child: isConnected ? const Text('断开') : const Text('连接'),
    );
  }
    ...
    return ExpansionTile(
      ...
      children: <Widget>[
        if (adv.advName.isNotEmpty)
          _buildAdvRow(context, '完整的本地名称:', adv.advName),
        if (adv.txPowerLevel != null)
          _buildAdvRow(context, '功率:', '${adv.txPowerLevel}'),
        if (adv.manufacturerData.isNotEmpty)
          _buildAdvRow(
              context, '厂商数据:', getNiceManufacturerData(adv.manufacturerData)),
        if (adv.serviceUuids.isNotEmpty)
          _buildAdvRow(
              context, '服务UUIDs', getNiceServiceUuids(adv.serviceUuids)),
        if (adv.serviceData.isNotEmpty)
          _buildAdvRow(context, '服务数据:', getNiceServiceData(adv.serviceData)),
      ],
    );
  }
}
